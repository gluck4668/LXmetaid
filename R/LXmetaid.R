...
gen_back_symbol <- gen_back_lst_all[,c("gene_id","pvalue","FC")]
colnames(gen_back_symbol) <- c("Gene_symbol","pvalue","FoldChange")

gen_back_lst <- dplyr::inner_join(gen_set0,entr_id_back,by = "gene_id") %>% na.omit()
gen_back_lst <- gen_back_lst[,c("ENTREZID","pvalue","FC")]
colnames(gen_back_lst) <- c("Gene_ENTREZID","pvalue","FoldChange")

write.table(gen_back_symbol,paste0(dir.name,"/LRpath_Concepts (background_genes)_symbol.txt"),row.names = F,quote = F,sep ="\t")
#write.table(gen_back_lst,paste0(dir.name,"/LRpath_Concepts (background_genes)_ENTREZID.txt"),row.names = F,quote = F,sep ="\t")
...
#write.csv(gen_df_all,paste0(dir.name,"/Genes_lst_all.csv"),row.names = F)
write.csv(gene_lst,paste0(dir.name,"/Genes_lst_(for MetScape).csv"),row.names = F)
...
#write.csv(met_df_all,paste0(dir.name,"/Compounds(metabolite)_lst_all.csv"),row.names = F)
write.csv(met_kegg,paste0(dir.name,"/Compounds (metabolite)_lst_(for MetScape).csv"),row.names = F)
